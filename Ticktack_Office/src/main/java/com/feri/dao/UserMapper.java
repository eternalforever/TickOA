...
public interface UserMapper {
	...
	//查询用户对应的菜单权限
	@Select("select a.id,a.title  from t_authority a left join t_roleauthority ra on a.id=ra.aid left join t_userrole ur on ra.rid=ur.rid  where a.type=1 and a.parentId=0 and ur.uid=#{id}")
	...
	public List<UserRoleVo> queryAll(String tj);
